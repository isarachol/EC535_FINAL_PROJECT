...
static int calc_duty(int state){
    ...
    // printk(KERN_DEBUG "walker: DEBUG: Calc Duty State=%d -> Duty=%d ns\n", state, duty); 
    return duty;
}
...
static void timer_callback(struct timer_list *t){
    ...
    if (vision_is_fresh) {
        ...
        if (current_mode == VISION_CONTROL_MODE) {
            // Valid "GO" command from vision
            // We MUST call move() to animate the walker's gait
            // move() uses the global pwm0_state (steering) which was set in dev_write
            ...
            move(); // <--- CRITICAL FIX: Advance the walking cycle!
        ...
        // Runs only if vision timed out
        
        if (current_mode == VISION_CONTROL_MODE) {
             printk(KERN_INFO "walker: Vision signal lost (Timeout). Reverting.\n");
             current_mode = 0; // Default to stop on timeout for safety
        }
                            ...
                            size_t len, loff_t *offset)
{
    ...
    if (sscanf(command_buffer, "%15[^:]:%d", command_verb, &angle_value) == 2) {
        ...
        if (strcmp(command_verb, "FORWARD") == 0) {
            ...
            // Maps 45->-5 (Left), 90->0 (Center), 135->5 (Right)
            pwm0_state = (angle_value - 90) / 9; 
            
            // 2. Set mode to VISION (but do NOT set PWMs here, leave that to move() in timer_callback)
            current_mode = VISION_CONTROL_MODE;
            
            // printk(KERN_DEBUG "walker: Vision Set Steering to %d\n", pwm0_state);
