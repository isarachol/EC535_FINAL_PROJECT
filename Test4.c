...
static int pwm1_state = 0; // Left Leg: -10 to 10
static int pwm2_state = 0; // Right Leg: -10 to 10
...
static int current_mode = 0; // 0=stop, 1=forward, 2=left, 3=right, 9=VISION
...
static void actuate(void) {
    ...
    // effective_state = state (-10 or 10) * amplitude (0 to 10) / 10
    int eff_state1 = (pwm1_state * amplitude_L) / 10;
    int eff_state2 = (pwm2_state * amplitude_R) / 10;

    int duty1 = duty_center + (eff_state1 * duty_span / 10);
    int duty2 = duty_center + (eff_state2 * duty_span / 10);
...
static void timer_callback(struct timer_list *t){
    ...
    if (echoed == 0) {
        ...
        dist_cm = 0; 
        // printk(KERN_WARNING "walker: Sensor Timeout - Safety Stop\n");
    }
    ...
    // 3. CONTROL LOGIC
    if (vision_is_fresh && current_mode == VISION_CONTROL_MODE) {
        // --- VISION MODE ---
        ...
        if (current_mode == VISION_CONTROL_MODE) {
             ...
             current_mode = 0; 
             amplitude_L = 10; amplitude_R = 10; // Reset stride
        }
        ...
        if (dist_cm <= dist_goal && dist_cm > 0) { 
            ...
            current_mode = 1;
            // Auto mode defaults to straight
            pwm0_state = 0;
            amplitude_L = 10;
            amplitude_R = 10;
            move();
...
static irqreturn_t btn0_handler(int irq, void *dev_id) { return IRQ_HANDLED; }
static irqreturn_t btn1_handler(int irq, void *dev_id) { return IRQ_HANDLED; }
                         ...
                         size_t len, loff_t *offset)
{
    ...
    if (sscanf(command_buffer, "%15[^:]:%d", command_verb, &angle_value) == 2) {
        ...
        if (strcmp(command_verb, "FORWARD") == 0) {
            ...
            // 90 is center (0)
            int steer = (angle_value - 90) / 9;
            pwm0_state = steer;
            
            // Differential Steering Logic
            if (angle_value < 80) { // Left turn
                amplitude_L = 5; 
                amplitude_R = 10;
            } else if (angle_value > 100) { // Right turn
                amplitude_L = 10;
                amplitude_R = 5; 
            } else {
                amplitude_L = 10;
...
static int __init walker_init(void)
{
    ...
    if (request_irq(irq_btn0, btn0_handler, IRQF_TRIGGER_RISING, "btn0_irq", NULL)) return -1;
    if (request_irq(irq_btn1, btn1_handler, IRQF_TRIGGER_RISING, "btn1_irq", NULL)) return -1;
