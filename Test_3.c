...
// PWM IDs (Check your wiring!)
// ID 0 = P9_22 (EHRPWM0A)
// ID 1 = P9_21 (EHRPWM0B)
// ID 4 = P9_14 (EHRPWM1A)
// ID 5 = P9_16 (EHRPWM1B)
#define pwm0_id 0 // CHANGED: 0 corresponds to P9_22 (Steering)
...
static int walker_init(void)
{
    ...
    // Request pwm
    pwm0 = pwm_request(pwm0_id, "pwm0"); // FIX: Use configured pwm0_id (0)
    if (IS_ERR(pwm0)) {
        pr_info("pwm0 failed\n");
        // ... (cleanup code)
        return PTR_ERR(pwm0);
    ...
    pwm1 = pwm_request(pwm1_id, "pwm1"); // FIX: Use configured pwm1_id
    if (IS_ERR(pwm1)) {
        pr_info("pwm1 failed\n");
        // ... (cleanup code)
        pwm_free(pwm0);
